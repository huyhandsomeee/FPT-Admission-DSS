...
// 1. Convert static arrays to states
code = code.replace(
  '  const tuitionDebts = [',
...
if (!code.includes('TUITION REMINDER MODAL')) {
  // Try matching different endings
  if (code.includes('    </div>\n  );\n}\nexport default FinanceOfficerPortal;')) {
    ...
    // Just replace the last </div>
    const lastDivIndex = code.lastIndexOf('</div>');
    if (lastDivIndex !== -1) {
       code = code.substring(0, lastDivIndex) + modals + code.substring(lastDivIndex);
    }
  }
}
