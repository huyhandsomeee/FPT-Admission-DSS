...
export default function RiskMonitor() {
  ...
  const loadRisks = () => {
    api.get("/api/manager/risks")
      ...
      .catch(err => console.error("Lỗi khi tải danh sách rủi ro:", err));
  };
  ...
  const displayRisks = risks.length > 0 ? risks : MOCK_RISKS;

  const activeHigh = displayRisks.filter(r => r.level === "HIGH" && r.status === "ACTIVE").length;
  const activeMed = displayRisks.filter(r => r.level === "MEDIUM" && r.status === "ACTIVE").length;
  const activeLow = displayRisks.filter(r => r.level === "LOW" && r.status === "ACTIVE").length;

  const riskSummary = [
    { count: activeHigh, label: "RỦI RO CAO", badge: "CRITICAL", bg: "#FEF2F2", color: "#DC2626", badgeBg: "#FEE2E2", barColor: "#DC2626", barPct: activeHigh * 30 },
    { count: activeMed, label: "RỦI RO VỪA", badge: "MEDIUM", bg: "#FFFBEB", color: "#D97706", badgeBg: "#FEF3C7", barColor: "#D97706", barPct: activeMed * 30 },
    { count: activeLow, label: "RỦI RO THẤP", badge: "STABLE", bg: "#F0FDF4", color: "#16A34A", badgeBg: "#DCFCE7", barColor: "#16A34A", barPct: activeLow * 30 },
  ];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {riskSummary.map((item) => (
          <div key={item.label} style={{
            ...
            <div style={{ width: "100%", height: 4, background: "#F1F5F9", borderRadius: 99 }}>
              <div style={{ width: `${Math.min(100, Math.max(10, item.barPct))}%`, height: "100%", background: item.barColor, borderRadius: 99 }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {displayRisks.map((risk, i) => {
            const IconComponent = getIcon(risk.iconType);
            return (
              <div key={i} style={{
                background: "white", borderRadius: 14, padding: "20px 22px",
                border: "1px solid #E8EDF5", boxShadow: "0 1px 6px rgba(0,0,0,0.05)",
                ...
                {/* Icon */}
                <div style={{ width: 42, height: 42, background: risk.levelBg, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <IconComponent size={20} color={risk.levelColor} />
                </div>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      {risk.status === "MITIGATED" && (
                        <span style={{ fontSize: 10, background: "#E6F4EA", color: "#137333", padding: "2px 6px", borderRadius: 4, fontWeight: 700 }}>
                          ✓ ĐÃ ỨNG PHÓ
                        </span>
                      )}
                    </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: "#0d1b3e", borderRadius: 16, padding: 28, position: "relative", overflow: "hidden" }}>
          ...
          <a href="/bod/recommendations" style={{
          ...
          <div style={{
            ...
            {selectedRisk.status === "MITIGATED" && (
              <span style={{
                ...
                padding: "3px 8px", borderRadius: 5, display: "inline-block", marginBottom: 14, marginLeft: 8
              }}>
                TRẠNG THÁI: ĐÃ KÍCH HOẠT ỨNG PHÓ
              </span>
            )}
            ...
            {selectedRisk.actionPlan && (
              <div style={{ fontSize: 13, color: "#374151", lineHeight: 1.6, marginBottom: 20, background: "#EFF6FF", padding: 12, borderRadius: 10 }}>
                <strong>Kế hoạch hành động cụ thể:</strong>
                <p style={{ margin: "6px 0 0", color: "#1D4ED8" }}>{selectedRisk.actionPlan}</p>
              </div>
            )}
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
              ...
              {selectedRisk.status !== "MITIGATED" && (
                <button 
                  onClick={() => handleMitigate(selectedRisk.id, selectedRisk.suggestion)}
                  style={{
