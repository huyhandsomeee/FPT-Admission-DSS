            ...
            {requests.map(req => (
              <div key={req.userId} style={{
                ...
                <div style={{ fontSize: 13, color: "#475569" }}>
                  Thí sinh: <strong style={{ color: "#1E293B" }}>{req.fullName}</strong> ({req.email}) — SĐT: <strong>{req.phone || "—"}</strong> — Mã HS cũ: <code style={{ background: "#F1F5F9", padding: "2px 6px", borderRadius: 4 }}>{req.studentCode}</code>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    onClick={() => handleAllowRequest(req.userId, true)}
                    style={{
                      padding: "6px 14px", background: "#10B981", border: "none",
                      color: "white", fontWeight: 700, fontSize: 12, borderRadius: 8, cursor: "pointer"
                    }}
                  >
                    Cho phép tạo
                  </button>
                  <button
                    onClick={() => handleAllowRequest(req.userId, false)}
                    style={{
