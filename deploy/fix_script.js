...
const fileSrc = fs.readFileSync('deploy/clean_localseed.js', 'utf-8');
const lines = fileSrc.split('\n');

const truncateIdx = lines.findIndex(l => l.includes('DỮ LIỆU BÙ ĐẮP'));
if (truncateIdx !== -1) {
    const originalScript = lines.slice(0, truncateIdx - 2).join('\n');
    let sql = `
let sql = '-- ==========================================================\\n';
sql += '-- DỮ LIỆU BÙ ĐẮP CHO CÁC BẢNG BI LỖI SCHEMA TRANSACTIONS\\n';
sql += '-- ==========================================================\\n\\n';
...
fsLib.appendFileSync("deploy/fix_seed.sql", sql, "utf8");
console.log("Appended fully parsed Mock data to deploy/fix_seed.sql");
`;
    fs.writeFileSync('deploy/clean_localseed.js', originalScript + '\n' + sql, 'utf8');
}
